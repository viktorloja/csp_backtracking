...
def build_base_timelines(barristers, day_start=0, day_end=24*60):
    ...
    for b in barristers:
        ...
        events = [(day_start, day_start, home, "HOME_START", None)]
        blocks = sorted(b.get("schedule", []), key=lambda x: x["start_time"])
        for blk in blocks:
            events.append((blk["start_time"], blk["end_time"], blk.get("location", home), "BLOCK", None))
        events.append((day_end, day_end, home, "HOME_END", None))
        base[bname] = events
    return base
...
def try_insert_case_get_delta(
    ...
    starts = [e[0] for e in timeline]  # could cache per barrister for speed; OK for moderate sizes
    idx = bisect_left(starts, c_start)
    ...
    prev_end, prev_loc = prev_ev[1], prev_ev[2]
    next_start, next_loc = next_ev[0], next_ev[2]
...
def branch_and_bound_travel(
    ...
    *,
    ...
    base_timelines = build_base_timelines(barristers, day_start, day_end)
    ...
    def order_values(case):
        ...
        for b in domains[case]:
            if b == UNASSIGNED:
                ...
                continue
            feasible, delta, idx, _, _ = try_insert_case_get_delta(case, b, timelines[b], cases_by_name, travel_times)
            if not feasible:
                continue
    ...
    def dfs(rem_cases, current_cost):
        ...
        for inc, b, delta, idx in candidates:
            ...
            if b != UNASSIGNED:
                ...
                ev = (c["time"], c["time"] + c["duration"], c["location"], "CASE", case)
